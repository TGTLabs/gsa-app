...
var polarCacheDurationSeconds = 60;
...
var getting = (function () {
  var cached = { data: null, expires: new Date(100).getTime() };
  ...
  return function () {
    if ((new Date()).getTime() <= cached.expires) {
      ...
      return pollData().then(function (data) {
        cached = {data: data, expires: (new Date()).getTime() + (1000 * polarCacheDurationSeconds) };
        return data;
      });
    }
  };
